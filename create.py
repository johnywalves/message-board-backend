...
def createDatabase():
    ...
    # MySQL-Python is available at: http://sourceforge.net/projects/mysql-python
    # mysql+mysqldb://<user>:<password>@<host>[:<port>]/<dbname>
    # engine = create_engine('sqlite:///:memory:', echo=True)   
    #engine = create_engine('mysql://root:masterkey@localhost:3306/mydb', poolclass=SingletonThreadPool) 

    Base.metadata.create_all(engine)
    Session = sessionmaker(bind=engine)

    # Insert Test
    user_admin = User(user='admin', password='123')
